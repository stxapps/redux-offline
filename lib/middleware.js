  ...
  return function (store) {
    return function (next) {
      return function (action) {
        ...
        var dispatchFunction = config.dispatch || store.dispatch; // create promise to return on enqueue offline action
        ...
        if (offlineAction && !offline.busy && !offline.retryScheduled && offline.online) {
          (0, _send.default)(offlineAction, dispatchFunction, config, offline.retryCount);
        }

        if (action.type === _constants.OFFLINE_SCHEDULE_RETRY) {
          after(action.payload.delay).then(function () {
            dispatchFunction((0, _actions.completeRetry)(offlineAction));
          });
        }
