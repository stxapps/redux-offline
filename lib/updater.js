...
var buildOfflineUpdater = function buildOfflineUpdater(dequeue, enqueue, filterOutboxRehydrate) {
  return function offlineUpdater() {
    ...
    if (action.type === _constants.PERSIST_REHYDRATE && action.payload) {
      var newState = _objectSpread(_objectSpread(_objectSpread({}, state), action.payload.offline || {}), {}, {
        online: state.online,
...
var enhanceReducer = function enhanceReducer(reducer, config) {
  var queue = config.queue,
      filterOutboxRehydrate = config.filterOutboxRehydrate;
  var dequeue = queue.dequeue,
      enqueue = queue.enqueue;
  var offlineUpdater = buildOfflineUpdater(dequeue, enqueue, filterOutboxRehydrate);
